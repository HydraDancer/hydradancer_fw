...
void hspi_rx_callback(uint8_t* buffer, uint16_t size, uint16_t custom_register)
{
	...
	LOG_IF(LOG_LEVEL_TRACE, LOG_ID_USER, "hspi_rx_callback buffer %p size %d ep %d request_code_bit %d request_code %d register %d \r\n", buffer, size, endpoint, request_code_bit, request_code, custom_register);
	...
	else
	{
		...
		{
			LOG_IF(LOG_LEVEL_DEBUG, LOG_ID_USER, "CONNECTION_REQUEST \r\n");
			usb2_device_init();
		}
		else if (endpoint == 0 && request_code_bit && request_code == HSPI_SERDES_DISABLE_USB)
		{
			LOG_IF(LOG_LEVEL_DEBUG, LOG_ID_USER, "DISCONNECT REQUEST \r\n");
			usb2_device_deinit();
			reinit_emulated_board();
			return;
		}
		else if (endpoint == 0 && request_code_bit && request_code == HSPI_SERDES_EMULATION_BOARD_BUS_RESET)
		{
			LOG_IF(LOG_LEVEL_DEBUG, LOG_ID_USER, "DO_BUS_RESET\r\n");
			do_bus_reset();
			return;
		}
		...
		{
			LOG_IF(LOG_LEVEL_DEBUG, LOG_ID_USER, "Set address %d \r\n", buffer[0]);
...
int main()
{
	...
	if (err > 0)
		...
		LOG_IF(LOG_LEVEL_DEBUG, LOG_ID_USER, "SYNC Err Timeout\n");
	...
	*/
	...
	usb2_user_handled.usb2_device_handle_bus_reset = usb2_device_handle_bus_reset;
	...
	LOG_IF_LEVEL(LOG_LEVEL_INFO, "emulated board");
