...
__attribute__((always_inline)) static inline uint8_t usb_control_endp_rx_callback(uint8_t* const ptr, uint16_t size, uint8_t endp_num)
{
	...
	else
	{
		...
		hydra_interrupt_queue_set_next_task(_usb0_ep0_send, (uint8_t*)ep0_queue_member, _ep0_queue_cleanup, INTERRUPT_QUEUE_LOW_PRIO);
	}
	usb_device_1.endpoints.rx[endp_num].buffer = ramx_pool_alloc_bytes(ENDP_1_15_MAX_PACKET_SIZE);
	hydradancer_status.ep_in_status &= ~(0x01 << endpoint_mapping_reverse[endp_num]);
...
bool _do_bus_reset(uint8_t* data);
bool _do_bus_reset(uint8_t* data)
{
	...
	hydra_pool_clean(&ep0_queue);
	ramx_pool_init();
	bsp_enable_interrupt();
	usb_emulation_do_bus_reset();
	usb_control_reinit();
	return true;
}
...
uint16_t usb_control_endp0_user_handled_control_request(USB_SETUP* request, uint8_t** buf)
{
	...
	{
		hydra_interrupt_queue_set_next_task(_do_bus_reset, NULL, NULL, INTERRUPT_QUEUE_LOW_PRIO);
		return 0;
	}
